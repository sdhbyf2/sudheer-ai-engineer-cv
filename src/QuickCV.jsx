...
export default function QuickCV({ onClose }) {
 ...
 return <dialog ref={ref} className="quick-cv" aria-labelledby="quick-cv-title" onCancel={onClose} data-lenis-prevent>
  ...
  <div className="cv-sheet">
   ...
   <section className="cv-summary"><h3>Profile</h3><p>Hands-on AI engineer working across architecture, backend, LLM integration, frontend, and deployment. Experience building RAG retrieval, multi-model routing with automatic failover, agentic workflows, and real-time voice assistants. Master’s in Data Science & AI, alongside an MBA.</p></section>
   ...
   <section><h3>Experience</h3>{roles.map(role=><article className="cv-role" key={role.company}><div><strong>{role.role}</strong><span>{role.date}</span></div><h4>{role.company} · {role.location}</h4><p>{role.summary} {role.detail}</p></article>)}</section>
