...
try {
 ...
 for (const name of (process.env.CV_TEST_BROWSERS || 'chromium,firefox,webkit').split(',')) {
  ...
  try {
   ...
   await expect(page.locator('#story .recruiter-details')).toContainText('valid until 2029');
   ...
   expect(new URL(await hiringLink.getAttribute('href')).searchParams.get('subject')).toBe('AI Engineer opportunity');
   ...
   await expect(page.locator('.quick-cv .recruiter-details')).toContainText('Full-time AI engineering roles');
